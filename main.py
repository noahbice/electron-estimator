    ...
    for j in range(8):
        ...
        for r_idx in range(len(ranges) + 1):
            if r_idx == 0:
                ...
                continue
            d_idx = np.where(np.abs(fine_res_dose[d_max_idx:] - ranges[r_idx - 1]) == np.amin(
                np.abs(fine_res_dose[d_max_idx:] - ranges[r_idx - 1])))[0]
            d_idx += d_max_idx
...
input_norm = input_cols[5].radio('Normalization: ',
                                 ('100% dose to 95% of volume', '100% to max depth', '100% to min depth'))
input_norm_dose = 0
input_norm_vol = 0
...
if filtered.empty:
    ...
    if out_cols[1].button('Proceed'):
        ...
        out_cols[1].table(output.drop(columns=['norm', 'Error']))
else:
    #out_cols[1].dataframe(filtered, width=2000, height=1000)
    out_cols[1].table(filtered)
    ...
    for bt in range(6):
        ...
        axs.set_xlim([0, np.minimum(3 * t_max_input + boluses[bt], data[e][-1, 0])])
        axs.set_ylim([0, 150])
